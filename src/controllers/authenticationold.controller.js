...
export const login = async (req, res) => {
  try {
    ...
    else if(data.login_type == TYPES.LOGIN_TYPE_GOOGLE || data.login_type == TYPES.LOGIN_TYPE_FACEBOOK || data.login_type == TYPES.LOGIN_TYPE_APPLE ) {
      /* Social login */
...
export const signUp = async (req, res) => {
  try {
    ...
    if (data.login_type == TYPES.LOGIN_TYPE_NORMAL) {
      ...
      if (data.email && data.phone_number) {
        filter = { $or: [{ email: data.email }, { phone_number: data.phone_number }] };
      }
      ...
      if (userDoc.length > 0) {
        ...
        let freeTrialUserDoc = {};
        await User.add(newUser).then(
          async (resp) => {
            if (file) {
              const filePath = Path.parse(file.originalname);
              const fileName = generateRandomString();
              const s3Location = `${UPLOAD_PATH.Profile}${resp._id}/${fileName}${filePath.ext}`;
              await uploadFileAndUpdateProfileURL(s3Location, file, { _id: resp._id });
              resp['profile_image'] = await getSignedUrl(
                process.env.Aws_Bucket_Name,
                s3Location
              );
            }
            await addOrUpdateDeviceTokens(resp._id, data);
            if (resp.email) {
              const OTP = await makeNumericId(6);
              logger.log(level.info, `Signup generated OTP=${OTP}`);
  ...
  try {
    ...
    if (userData.length > 0) {
      ...
      if (mail) {
        ...
        await User.update(
          { email: data.email || userData[0].email },
          insertMailId
        );
      }
...
async function generateToken(
  ...
  userDoc = null,
  paymentDoc = null,
  role = null // Added role parameter
) {
  ...
  // Pass the role to createToken
  const accessToken = await auth.createToken(email, doc._id, role);
...
async function uploadFileAndUpdateProfileURL(s3Location, file, filter) {
  ...
  await uploadFileToS3(process.env.Aws_Bucket_Name, s3Location, file).then(
    async (result) => {
...
export const makePayment = async (request, response) => {
  try {
    ...
    // return okResponse(response, "Payment successful", paymentIntent);
  } catch (error) {
